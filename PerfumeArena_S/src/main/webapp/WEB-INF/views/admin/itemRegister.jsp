...
<style type="text/css">
    ...
    body {
        background-color: #f5f9ff; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;
    }

    .page-title {
		width: 80%; max-width: 1200px; margin: 50px auto 10px auto; font-size: 28px; font-weight: bold;
        color: #2a4e8a; border-bottom: 3px solid #4a90e2; padding-bottom: 15px; text-align: left; padding-top:60px;
    }

    table#ItemInput {
        border-collapse: separate; border-spacing: 0; width: 80%; max-width: 1200px; margin: 0% auto; font-size: 16px;
        background-color: white; border-radius: 10px; box-shadow: 0 5px 15px rgba(74, 144, 226, 0.1); overflow: hidden;
    }

    table#ItemInput td {
        border: 1px solid #d4e3fc; padding: 15px; transition: background-color 0.3s;
    }
    ...
    .itemInputName {
        background-color: #e6f0ff; font-weight: bold; width: 25%; text-align: right;
        padding-right: 20px; color: #2a4e8a;
    }

    table#ItemInput td input[type="text"], table#ItemInput td input[type="file"], 
    table#ItemInput td select, table#ItemInput td textarea {
        width: 100%; max-width: 400px; box-sizing: border-box; padding: 8px 12px; border: 1px solid #c4d8f9;
        border-radius: 4px; background-color: #f9fcff; transition: all 0.3s;
    }

    table#ItemInput td input:focus, table#ItemInput td select:focus, table#ItemInput td textarea:focus {
        outline: none; border-color: #4a90e2; box-shadow: 0 0 0 2px rgba(74, 144, 226, 0.2);
    }
    ...
    #btnRegister:hover {
        background-color: #3a7bc8; border-color: #3a7bc8;
    }

    #previewImg {
        max-width: 100%; height: auto; border: 1px solid #d4e3fc; border-radius: 4px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);
    }

    @media (max-width: 768px) {
        .page-title { font-size: 22px; text-align: center; }        
        table#ItemInput { width: 95%; font-size: 14px; }        
        .itemInputName { width: 30%; padding-right: 10px; }        
        table#ItemInput td { padding: 10px 8px; }
    }

    @media (max-width: 480px) {
        .page-title { font-size: 20px; }        
        table#ItemInput { display: block; }        
        table#ItemInput tbody, table#ItemInput tr, table#ItemInput td 
        { display: block; width: 100% !important; text-align: left !important; }        
        .itemInputName { background-color: transparent; font-weight: bold; color: #2a4e8a; padding-bottom: 5px; }
    }
  	...
  	$(function(){
      	...
      	$(document).on('click', 'input:button[id="btnRegister"]', function(){
         	...
         	if(is_infoData_OK) {
            	...
            	$.ajax({
       				url:"<%= ctxPath%>/admin/itemRegister",
            		type:"post",
               		data:formData,
               		processData:false,
               		contentType:false,
               		...
               		success:function(json){
                  		if(json.result == 1) {
                    		location.href="<%= ctxPath%>/item/mallHome";
                  		}
               		},
